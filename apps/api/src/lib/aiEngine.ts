  ...
  "questions": [
    {
      ...
      try {
        ...
        logger.info('ai.assessment_questions_generated', {
          examSlug,
          language,
        ...
        logger.error('ai.assessment_questions_error', {
    ...
    async scoreAssessment(
      ...
      for (const answer of answers) {
        ...
        if (q && answer.chosen === q.correctOption) {
          correct++;
        }
      }
      ...
      // Use GPT-4o for personalized level assignment
      try {
        const prompt = `A student took an assessment for an Indian competitive exam.
They scored ${correct}/${total} (${percentage.toFixed(1)}%).

Difficulty breakdown of questions they got right:
${answers
  .map((a) => {
    const q = questions.find((q) => q.id === a.questionId);
    if (!q) return null;
    const isCorrect = a.chosen === q.correctOption;
    return `- ${q.difficulty}: ${isCorrect ? 'correct' : 'incorrect'}`;
  })
  .filter(Boolean)
  .join('\n')}
...
Respond ONLY with valid JSON:
{
  ...
  "messageHi": "Same message in Hindi (Devanagari)"
        ...
        return {
          ...
          messageHi: parsed.messageHi,
        };
      } catch (err) {
        logger.error('ai.assessment_score_error', {
          error: err instanceof Error ? err.message : String(err),
        });
        ...
        return {
          ...
          total,
          level,
          message: `You scored ${correct}/${total}. We've set your level to ${level}. Let's start learning!`,
          messageHi: `आपने ${correct}/${total} अंक प्राप्त किए। आपका स्तर ${level} निर्धारित किया गया है। चलिए सीखना शुरू करते हैं!`,
        };
      }
    },
  };
