...
export const api = {
  ...
  ai: {
    async generateMcqs(subject?: string, count?: number): Promise<{ mcqs: Array<{ question: string; options: { key: string; text: string }[]; correctOption: string; explanation: string; subject: string; difficulty: string }> }> {
      const res = await authedFetch('/v1/ai/mcqs', { method: 'POST', body: JSON.stringify({ subject, count }) });
      return res.json() as Promise<{ mcqs: Array<{ question: string; options: { key: string; text: string }[]; correctOption: string; explanation: string; subject: string; difficulty: string }> }>;
    },
    async generateChapter(topic: string): Promise<{ chapter: { title: string; sections: { heading: string; content: string }[]; summary: string; keyPoints: string[] } }> {
      ...
      return res.json() as Promise<{ chapter: { title: string; sections: { heading: string; content: string }[]; summary: string; keyPoints: string[] } }>;
    },
    async generateMockTest(subject?: string): Promise<{ id: string; mcqs: Array<{ question: string; options: { key: string; text: string }[]; correctOption: string; explanation: string; subject: string; difficulty: string }>; durationMinutes: number; totalQuestions: number }> {
      const res = await authedFetch('/v1/ai/mock-test', { method: 'POST', body: JSON.stringify({ subject }) });
      return res.json() as Promise<{ id: string; mcqs: Array<{ question: string; options: { key: string; text: string }[]; correctOption: string; explanation: string; subject: string; difficulty: string }>; durationMinutes: number; totalQuestions: number }>;
    },
    async searchNexipedia(topic: string): Promise<{ article: { title: string; summary: string; sections: { heading: string; content: string; imageQuery?: string }[]; relatedTopics: string[]; youtubeQuery: string; diagramPrompt: string } }> {
      ...
      return res.json() as Promise<{ article: { title: string; summary: string; sections: { heading: string; content: string; imageQuery?: string }[]; relatedTopics: string[]; youtubeQuery: string; diagramPrompt: string } }>;
    },
