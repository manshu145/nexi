...
export const api = {
  ...
  async getInterviewToken(opts: { role?: string; exam?: string; lang?: 'en' | 'hi' }) { return (await authedFetch('/v1/interview/token', { method: 'POST', body: JSON.stringify(opts) })).json() as Promise<{ token: string; model: string; lang: 'en' | 'hi'; systemInstruction?: string; availableModels?: string[]; diag?: string }>; },
