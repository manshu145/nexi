    ...
    <main className="shorts-shell fixed inset-0 flex flex-col overflow-hidden select-none">
      ...
      {filtered.length === 0 ? (
        ...
        <div
          ...
          onWheel={handleWheel}
        >
          {/* Desktop layout wrapper: center card + optional sidebar */}
          ...
          {/* Mobile progress dots (right side) */}
              ...
              return (
                <div
                  ...
                  className={`rounded-full transition-all duration-300 ${
                    realIdx === currentIdx ? 'w-1.5 h-4 bg-white' : 'w-1.5 h-1.5 bg-white/30'
                  }`}
                />
              );
            })}
  ...
  return (
    <div className="h-full w-full flex items-center justify-center px-3 py-2 lg:px-0 lg:py-3">
      ...
      >
        ...
        {/* Mobile action buttons (right edge, over card) */}
        <div className="absolute right-2 bottom-16 flex flex-col items-center gap-3 z-10 lg:hidden">
          ...
          <button onClick={(e) => { e.stopPropagation(); onBookmark(); }} className="flex flex-col items-center bg-black/20 backdrop-blur-sm rounded-full p-2">
            <span className={`text-lg ${bookmarked ? 'scale-110' : ''}`}>{bookmarked ? '🔖' : '📑'}</span>
          </button>
          <button onClick={(e) => { e.stopPropagation(); onShare(); }} className="flex flex-col items-center bg-black/20 backdrop-blur-sm rounded-full p-2">
            <span className="text-lg">↗️</span>
          </button>
          <button onClick={(e) => { e.stopPropagation(); onAskNexi(); }} className="flex flex-col items-center bg-black/20 backdrop-blur-sm rounded-full p-2">
