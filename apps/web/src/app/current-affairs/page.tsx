...
export default function CurrentAffairsShortsPage() {
  ...
  const handleShare = async (item: CurrentAffairsItem) => {
    ...
    if (navigator.share) {
      try { await navigator.share({ title: item.headline, text, url: window.location.origin + `/current-affairs/${item.id}` }); } catch {}
    } else {
          ...
          <button
            ...
            className={`flex items-center gap-1 whitespace-nowrap px-3 py-1.5 rounded-full text-xs font-medium transition-all ${
              activeTab === cat.key
                ...
                : 'bg-paper-300 text-ink-700 hover:bg-paper-400 hover:text-ink-900'
            }`}
          >
            <span>{cat.emoji}</span>
            <span>{cat.label}</span>
        ...
        >
          ...
          <div className="absolute right-2 top-1/2 -translate-y-1/2 flex flex-col gap-1 z-10 lg:hidden">
            {filtered.slice(Math.max(0, currentIdx - 3), currentIdx + 4).map((item, i) => {
              ...
              return (
                <div key={item.id} className={`rounded-full transition-all duration-300 ${realIdx === currentIdx ? 'w-1.5 h-4 bg-ink-900' : 'w-1.5 h-1.5 bg-muted-400'}`} />
              );
            })}
          </div>
...
function ShortCard({ item, isActive, liked, bookmarked, likeCount, onLike, onBookmark, onShare, onTap, onAskNexi }: ShortCardProps) {
  ...
  return (
    <div className="h-full w-full flex items-center justify-center px-3 py-2 lg:px-0 lg:py-3">
      <div
        className={`relative w-full h-full rounded-2xl lg:rounded-3xl overflow-hidden shadow-xl border border-line transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] cursor-pointer bg-paper-50 ${
      ...
      >
        ...
        <div className="absolute right-2 bottom-16 flex flex-col items-center gap-3 z-10 lg:hidden">
          <button onClick={(e) => { e.stopPropagation(); onLike(); }} className="flex flex-col items-center gap-0.5 bg-paper-50/70 dark:bg-ink-950/50 backdrop-blur-sm rounded-full p-2 border border-line/50">
          ...
          <button onClick={(e) => { e.stopPropagation(); onBookmark(); }} className="bg-paper-50/70 dark:bg-ink-950/50 backdrop-blur-sm rounded-full p-2 border border-line/50">
            <span className={`text-lg ${bookmarked ? 'scale-110' : ''}`}>{bookmarked ? '🔖' : '📑'}</span>
          </button>
          <button onClick={(e) => { e.stopPropagation(); onShare(); }} className="bg-paper-50/70 dark:bg-ink-950/50 backdrop-blur-sm rounded-full p-2 border border-line/50">
            <span className="text-lg">↗️</span>
          </button>
          <button onClick={(e) => { e.stopPropagation(); onAskNexi(); }} className="bg-paper-50/70 dark:bg-ink-950/50 backdrop-blur-sm rounded-full p-2 border border-line/50">
