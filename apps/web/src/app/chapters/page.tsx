 ...
 * No input needed — AI determines what to generate based on exam + skill level.
 */
function AutoGeneratedLibrary({ exam }: { exam: string }) {
  ...
  // Exam-specific core subjects that AI should generate
  ...
  const subjects = EXAM_SUBJECTS[exam] ?? EXAM_SUBJECTS['jee-main'] ?? ['Physics Basics', 'Chemistry Fundamentals', 'Mathematics Foundation', 'General Science', 'Logical Reasoning', 'English Comprehension'];

  useEffect(() => {
    // Auto-start generating first chapter
  ...
  async function generateNext(index: number) {
    ...
    try {
      ...
      setLocalChapters((prev) => [...prev, res.chapter]);
      // Auto-generate next after a small delay
