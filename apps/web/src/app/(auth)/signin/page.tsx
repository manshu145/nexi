    ...
    <main className="flex min-h-screen items-center justify-center px-4">
      <div className="paper-card w-full max-w-sm p-8 text-center">
        ...
        <div className="mt-6 flex items-center gap-3">
          <div className="h-px flex-1 bg-paper-300 dark:bg-ink-600" />
        ...
        <form onSubmit={handleEmailSubmit} className="mt-6 space-y-3 text-left">
          <div>
            <label htmlFor="email" className="text-xs font-medium text-ink-700 dark:text-paper-200">Email</label>
            <input
              id="email"
              type="email"
              value={email}
          ...
          <div>
            <label htmlFor="password" className="text-xs font-medium text-ink-700 dark:text-paper-200">Password</label>
            <input
              id="password"
              type="password"
              value={password}
          ...
          {mode === 'signup' && (
            <div>
              <label htmlFor="confirmPassword" className="text-xs font-medium text-ink-700 dark:text-paper-200">Confirm Password</label>
              <input
                id="confirmPassword"
                type="password"
        ...
        <p className="mt-5 text-sm text-muted-500">
          {mode === 'signin' ? (
            <>Don&apos;t have an account?{' '}
              <button type="button" onClick={() => { setMode('signup'); setError(null); }} className="font-medium text-ember-600 dark:text-gold-500 hover:underline">
                Sign Up
              </button>
            </>
          ) : (
            <>Already have an account?{' '}
              <button type="button" onClick={() => { setMode('signin'); setError(null); }} className="font-medium text-ember-600 dark:text-gold-500 hover:underline">
                Sign In
              </button>
            </>
          )}
