...
export default function ExamPage() {
  ...
  for (const exam of EXAMS) { const existing = categories.get(exam.category) ?? []; existing.push(exam); categories.set(exam.category, existing); }
        ...
        {Array.from(categories.entries()).map(([category, exams]) => (
          <div key={category}>
            ...
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
              {exams.map((exam) => (
                <button key={exam.id} type="button" onClick={() => setSelected(exam.id)}
                  className={`rounded-lg border px-3 py-3 text-left text-sm font-medium transition-all ${selected === exam.id ? 'border-amber-500 bg-amber-50 text-amber-700 ring-2 ring-amber-500/20 dark:bg-amber-500/10 dark:text-amber-400' : 'border-slate-200 text-slate-700 hover:border-slate-300 dark:border-slate-700 dark:text-slate-300'}`}>
                  {exam.name}
                </button>
              ))}
            </div>
