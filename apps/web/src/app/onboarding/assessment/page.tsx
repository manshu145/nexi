...
export default function AssessmentPage() {
  ...
  const startAssessment = async () => {
    ...
    setError(null);
    try {
      const language = (localStorage.getItem('nexigrate-language') as 'en' | 'hi') || 'en';
      // Get user's target exam from API
  ...
  const submitAssessment = async () => {
    ...
    try {
      const answerArray = questions.map((q) => ({
        questionId: q.id,
        chosen: answers.get(q.id) ?? null,
      }));
      const result = await api.submitAssessment(questions, answerArray);
      // Store result temporarily for the complete page
  ...
  return (
    <div className="flex flex-col items-center">
            ...
            <button
              ...
              className={`w-full rounded-lg border px-4 py-3 text-left text-sm transition-all ${
                selectedAnswer === opt.key
                  ...
                  : 'border-slate-200 text-slate-700 hover:border-slate-300 hover:bg-slate-50 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-700/50'
              }`}
            >
      ...
      >
        {currentIndex >= questions.length - 1 ? 'Submit' : tc('next')}
      </button>
    </div>
  );
}
