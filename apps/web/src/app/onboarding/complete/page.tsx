    ...
    <div className="flex flex-col items-center">
      ...
      <div className="mt-12 text-center">
        ...
        {result && (<>
          ...
          <p className={`mt-2 text-xl font-bold ${levelColor[result.level]}`}>{t('level', { level: t(result.level) })}</p>
          <p className="mt-4 text-sm text-slate-500 dark:text-slate-400 leading-relaxed">{lang === 'hi' ? result.messageHi : result.message}</p>
        </>)}
      </div>
