...
export default function CompletePage() {
  ...
  const language = typeof window !== 'undefined'
    ? (localStorage.getItem('nexigrate-language') as 'en' | 'hi') || 'en'
    : 'en';

  const levelLabel = result
    ? t(result.level as 'beginner' | 'intermediate' | 'advanced')
    : '';
      ...
      <div className="mt-12 text-center">
        ...
        {result && (
          <>
            ...
            <p className={`mt-2 text-xl font-bold ${levelColor[result.level]}`}>
              {t('level', { level: levelLabel })}
            </p>
            <p className="mt-4 text-sm text-slate-500 dark:text-slate-400 leading-relaxed">
              {language === 'hi' ? result.messageHi : result.message}
            </p>
          </>
        )}
      </div>
