    ...
    <main className="mx-auto flex min-h-dvh max-w-4xl flex-col px-5 pt-6 pb-24 overflow-x-hidden">
      ...
      <section className="mt-6 animate-fadeIn-delay-1">
        ...
        <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
          ...
          {/* Jobs & Eligibility */}
          <button type="button" onClick={() => { track('feature_click', { feature: 'jobs' }); router.push('/jobs'); }} className="paper-card card-selectable group flex h-full flex-col p-4 text-left transition-all hover:shadow-md">
            <div className="flex items-center gap-2">
              <span className="inline-flex h-10 w-10 items-center justify-center rounded-xl bg-ember-500/10 text-ember-600"><IconBriefcase className="h-5 w-5" /></span>
              <span className="inline-flex items-center gap-1 rounded-full bg-ember-500/10 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-ember-600">New</span>
            </div>
            <h3 className="mt-3 font-serif text-base font-bold text-ink-900">{me?.language === 'hi' ? 'नौकरियाँ' : 'Jobs'}</h3>
            <p className="mt-1 text-xs text-muted-500 line-clamp-2">{me?.language === 'hi' ? 'देखें आप किन नौकरियों के लिए पात्र हैं' : 'See which jobs you are eligible for'}</p>
            <div className="mt-auto pt-2.5 flex items-center gap-1 text-xs font-medium text-ember-500">{t('read')} <span className="transition-transform group-hover:translate-x-0.5">→</span></div>
          </button>
...
function IconBriefcase({ className }: IconProps) {
  return <Svg className={className}><rect x="2.5" y="7" width="19" height="13" rx="2" /><path d="M9 7V5a2 2 0 012-2h2a2 2 0 012 2v2M2.5 12.5h19" /></Svg>;
}
