...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      } catch (e) { if (cancelled) return; setError(e instanceof Error ? e.message : 'Failed to load'); }
  ...
  const greeting = (() => { const h = (new Date().getUTCHours() + 5.5) % 24; if (h < 12) return t('greeting.morning'); if (h < 17) return t('greeting.afternoon'); return t('greeting.evening'); })();
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-2xl flex-col px-4 pt-8 pb-16">
      ...
      <section className="mt-8 grid gap-4 sm:grid-cols-3">
        <button type="button" onClick={() => router.push('/study')} className="card cursor-pointer text-left transition-all hover:border-amber-500/50 hover:shadow-md active:scale-[0.98]">
          <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-blue-100 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400"><svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" /></svg></div>
        ...
        <button type="button" onClick={() => router.push('/current-affairs')} className="card cursor-pointer text-left transition-all hover:border-amber-500/50 hover:shadow-md active:scale-[0.98]">
          <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-green-100 text-green-600 dark:bg-green-900/30 dark:text-green-400"><svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v1m2 13a2 2 0 01-2-2V7m2 13a2 2 0 002-2V9a2 2 0 00-2-2h-2m-4-3H9M7 16h6M7 8h6v4H7V8z" /></svg></div>
        ...
        <button type="button" onClick={() => router.push('/chat')} className="card cursor-pointer text-left transition-all hover:border-amber-500/50 hover:shadow-md active:scale-[0.98]">
